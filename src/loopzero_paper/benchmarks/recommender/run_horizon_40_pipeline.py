...
TARGET_HORIZON = 40
PACKET_NAME = f"{BENCHMARK_ID}__horizon_40_packet"
...
def build_repo_paths(repo_root: Path) -> RepoPaths:
    return RepoPaths(
        ...
        backup_dir=repo_root / "results" / "robustness" / "recommender" / "_tmp_restore_h40",
...
def archive_packet(paths: RepoPaths) -> None:
    ...
    metadata = {
        ...
        "note": (
            "This packet was generated by temporarily overriding max_horizon_steps to 40, rerunning the full recommender pipeline, "
...
def run(repo_root: Path) -> Path:
    ...
    print(f"[ok] archived horizon-40 packet: {paths.packet_dir}")
